...
#define UVC_EXT_PRIVACY_STUB_UNIT	0x7ffd
#define UVC_EXT_PRIVACY_STUB_UNIT_ID	0x200

#define UVC_HARDWARE_ENTITY_ID(id)	((id) & 0xff)
#define UVC_INVALID_ENTITY_ID		0xffff

/*
 * Purely-virtual entity/control implementing software frame substitution.
 * The GUID is never sent over the wire (get_cur/set_cur bypass the real UVC
 * control transport), it only needs to be unique within this driver's own
 * control tables.
 */
#define UVC_GUID_EXT_PRIVACY_STUB \
	{'U',  'V',  'C',  'P',  'R',  'I',  'V',  'S', \
	 'T',  'U',  'B',  0x00, 0x00, 0x00, 0x00, 0x00}
#define UVC_CT_PRIVACY_STUB_CONTROL	0x01

/* Custom V4L2 control ID for the privacy-stub toggle, see uvc_ctrl.c. */
#define V4L2_CID_UVC_PRIVACY_STUB	(V4L2_CID_USER_UVC_BASE + 3)

/* Global flag: when set, uvc_video.c substitutes stub frames for real ones. */
extern atomic_t uvc_privacy_stub_active;

/*
 * Load/unload the stub firmware matching the currently negotiated format.
 * Defined in uvc_driver.c; also called from uvc_v4l2.c on S_FMT to keep the
 * cached stub in sync if the format changes while privacy is active.
 */
struct uvc_device;
...
struct uvc_entity {
	...
	int (*set_cur)(struct uvc_device *dev, struct uvc_entity *entity,
		       u8 cs, void *data, u16 size);
...
struct uvc_device {
	...
	struct uvc_entity *privacy_stub_unit;

	/*
	 * Currently loaded stub frame, valid only while privacy is active.
	 * privacy_stub_lock protects the four fields below against the
	 * frame-substitution hot path (uvc_queue_buffer_complete(), which
	 * can run from URB-completion/softirq context) racing against
	 * privacy_stub being toggled from process context.
	 */
	spinlock_t privacy_stub_lock;
	const struct firmware *privacy_stub_fw;
	u32 privacy_stub_fw_fcc;
	u16 privacy_stub_fw_width;
	u16 privacy_stub_fw_height;
};
