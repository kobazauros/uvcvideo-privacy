			   ...
			   struct v4l2_format *fmt)
{
	...
	/*
	 * If privacy is already active, the cached stub was loaded against
	 * whatever format was current at activation time (possibly just the
	 * probe-time default, if activated before any app opened the
	 * device). Refresh it now so substituted frames match the format
	 * buffers are actually sized for. On failure, drop the stale entry
	 * rather than keep serving a stub sized/formatted for the old
	 * negotiation - the hot path falls back to blanking instead.
	 */
	if (atomic_read(&uvc_privacy_stub_active) &&
	    uvc_privacy_stub_load_fw(stream->dev) < 0)
		uvc_privacy_stub_unload_fw(stream->dev);
