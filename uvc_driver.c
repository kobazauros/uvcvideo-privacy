...
static const u8 uvc_privacy_stub_guid[16] = UVC_GUID_EXT_PRIVACY_STUB;
static const u8 uvc_media_transport_input_guid[16] =
	UVC_GUID_UVC_MEDIA_TRANSPORT_INPUT;
static const u8 uvc_processing_guid[16] = UVC_GUID_UVC_PROCESSING;

/* Global flag: when set, uvc_video.c substitutes stub frames for real ones. */
atomic_t uvc_privacy_stub_active = ATOMIC_INIT(0);
					       ...
					       unsigned int extra_size)
{
	...
	case UVC_EXT_GPIO_UNIT:
		...
		break;
	case UVC_EXT_PRIVACY_STUB_UNIT:
		memcpy(entity->guid, uvc_privacy_stub_guid, 16);
		break;
	case UVC_ITT_CAMERA:
...
/*
 * Purely-virtual privacy-stub control: get_cur/set_cur/get_info operate on
 * uvc_privacy_stub_active directly, no USB transfer is ever issued for it.
 */
static int uvc_privacy_stub_get_cur(struct uvc_device *dev,
				    struct uvc_entity *entity,
				    u8 cs, void *data, u16 size)
{
	if (cs != UVC_CT_PRIVACY_STUB_CONTROL || size < 1)
		return -EINVAL;

	*(u8 *)data = atomic_read(&uvc_privacy_stub_active) ? 1 : 0;

	return 0;
}

/*
 * Load the stub firmware matching the video streaming interface's currently
 * negotiated format/resolution into dev->privacy_stub_fw. Idempotent: if the
 * cached firmware already matches, this is a no-op. Called from process
 * context only (V4L2 SET_CTRL and S_FMT), request_firmware() is allowed to
 * sleep here. Also called from uvc_v4l2.c's S_FMT handler when privacy is
 * already active, so a format renegotiation (e.g. an app opening the device
 * after privacy was turned on against the probe-time default format) doesn't
 * leave a stale, wrongly-sized/format stub cached against the new buffers.
 */
int uvc_privacy_stub_load_fw(struct uvc_device *dev)
{
	struct uvc_streaming *stream, *video_stream = NULL;
	const struct firmware *fw, *old_fw;
	unsigned long flags;
	char path[64];
	u32 fcc;
	u16 width, height;
	...
	if (!video_stream || !video_stream->cur_format || !video_stream->cur_frame)
		return -ENODEV;

	fcc = video_stream->cur_format->fcc;
	width = video_stream->cur_frame->wWidth;
	height = video_stream->cur_frame->wHeight;

	if (dev->privacy_stub_fw && dev->privacy_stub_fw_fcc == fcc &&
	    dev->privacy_stub_fw_width == width &&
	    dev->privacy_stub_fw_height == height)
		return 0;

	snprintf(path, sizeof(path), "uvc_privacy/stub_%ux%u.%s", width, height,
		 (fcc == V4L2_PIX_FMT_MJPEG || fcc == V4L2_PIX_FMT_JPEG) ?
		 "jpg" : "yuyv");

	ret = request_firmware(&fw, path, &dev->intf->dev);
	if (ret < 0) {
		dev_warn(&dev->intf->dev,
			 "Privacy stub: failed to load %s (%d)\n", path, ret);
		return ret;
	}

	spin_lock_irqsave(&dev->privacy_stub_lock, flags);
	old_fw = dev->privacy_stub_fw;
	dev->privacy_stub_fw = fw;
	dev->privacy_stub_fw_fcc = fcc;
	dev->privacy_stub_fw_width = width;
	dev->privacy_stub_fw_height = height;
	spin_unlock_irqrestore(&dev->privacy_stub_lock, flags);

	if (old_fw)
		release_firmware(old_fw);

	return 0;
}

void uvc_privacy_stub_unload_fw(struct uvc_device *dev)
{
	const struct firmware *old_fw;
	unsigned long flags;

	spin_lock_irqsave(&dev->privacy_stub_lock, flags);
	old_fw = dev->privacy_stub_fw;
	dev->privacy_stub_fw = NULL;
	dev->privacy_stub_fw_fcc = 0;
	dev->privacy_stub_fw_width = 0;
	dev->privacy_stub_fw_height = 0;
	spin_unlock_irqrestore(&dev->privacy_stub_lock, flags);
...
static int uvc_privacy_stub_set_cur(struct uvc_device *dev,
				    struct uvc_entity *entity,
				    u8 cs, void *data, u16 size)
{
	int ret;
	...
	if (*(u8 *)data) {
		/*
		 * Only flip the flag once the stub frame is actually loaded,
		 * so activation fails loudly (SET_CTRL returns an error)
		 * instead of silently reporting "active" while still leaking
		 * the real feed.
		 */
		ret = uvc_privacy_stub_load_fw(dev);
		if (ret < 0)
			return ret;
		atomic_set(&uvc_privacy_stub_active, 1);
	} else {
		/* Clear the flag before freeing, see privacy_stub_lock. */
		atomic_set(&uvc_privacy_stub_active, 0);
		uvc_privacy_stub_unload_fw(dev);
	}

	return 0;
}

static int uvc_privacy_stub_get_info(struct uvc_device *dev,
				     struct uvc_entity *entity,
				     u8 cs, u8 *caps)
{
	if (cs != UVC_CT_PRIVACY_STUB_CONTROL)
		return -EINVAL;

	*caps = UVC_CONTROL_CAP_GET | UVC_CONTROL_CAP_SET;
	return 0;
}

static int uvc_privacy_stub_parse(struct uvc_device *dev)
{
	struct uvc_entity *unit;

	unit = uvc_alloc_new_entity(dev, UVC_EXT_PRIVACY_STUB_UNIT,
				    UVC_EXT_PRIVACY_STUB_UNIT_ID, 0, 1);
	if (IS_ERR(unit))
		return PTR_ERR(unit);

	unit->stub.bControlSize = 1;
	unit->stub.bmControls = (u8 *)unit + sizeof(*unit);
	unit->stub.bmControls[0] = 1;
	unit->get_cur = uvc_privacy_stub_get_cur;
	unit->set_cur = uvc_privacy_stub_set_cur;
	unit->get_info = uvc_privacy_stub_get_info;
	strscpy(unit->name, "Privacy Stub", sizeof(unit->name));

	list_add_tail(&unit->list, &dev->entities);

	dev->privacy_stub_unit = unit;
...
static int uvc_scan_device(struct uvc_device *dev)
{
	...
	/* Add the privacy-stub entity to the first chain. */
	if (dev->privacy_stub_unit) {
		chain = list_first_entry(&dev->chains,
					 struct uvc_video_chain, list);
		list_add_tail(&dev->privacy_stub_unit->chain, &chain->entities);
	}
		     ...
		     const struct usb_device_id *id)
{
	...
	spin_lock_init(&dev->privacy_stub_lock);
	...
	/* Register the software-only privacy-stub control. */
	ret = uvc_privacy_stub_parse(dev);
	if (ret < 0)
		goto error;
