...
static void uvc_queue_buffer_complete(struct kref *ref)
{
	...
	/*
	 * Privacy stub substitution: swap the completed frame's content for
	 * the pre-loaded stub before it reaches userspace. Only the main
	 * video-capture queue carries pixel data (the metadata queue shares
	 * this same completion path but must not be touched here, and
	 * uvc_queue_to_stream() below assumes queue == &stream->queue).
	 */
	if (!buf->error && queue->queue.type == V4L2_BUF_TYPE_VIDEO_CAPTURE &&
	    atomic_read(&uvc_privacy_stub_active)) {
		struct uvc_device *dev = uvc_queue_to_stream(queue)->dev;
		unsigned long flags;

		spin_lock_irqsave(&dev->privacy_stub_lock, flags);
		if (dev->privacy_stub_fw) {
			size_t n = min_t(size_t, dev->privacy_stub_fw->size,
					 buf->length);

			memcpy(buf->mem, dev->privacy_stub_fw->data, n);
			buf->bytesused = n;
		} else {
			/* No stub loaded for this format: fail closed. */
			memset(buf->mem, 0, buf->bytesused);
		}
