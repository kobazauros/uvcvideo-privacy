...
static const struct uvc_control_info uvc_ctrls[] = {
	...
	{
		.entity		= UVC_GUID_EXT_PRIVACY_STUB,
		.selector	= UVC_CT_PRIVACY_STUB_CONTROL,
		.index		= 0,
		.size		= 1,
		.flags		= UVC_CTRL_FLAG_SET_CUR | UVC_CTRL_FLAG_GET_CUR
				| UVC_CTRL_FLAG_RESTORE,
				  ...
				  struct uvc_control **err_ctrl)
{
	...
	for (i = 0; i < entity->ncontrols; ++i) {
		...
		if (!rollback) {
			if (ctrl->entity->set_cur)
				ret = ctrl->entity->set_cur(dev, ctrl->entity,
					ctrl->info.selector,
					uvc_ctrl_data(ctrl, UVC_CTRL_DATA_CURRENT),
					ctrl->info.size);
			else
...
static int uvc_ctrl_init_chain(struct uvc_video_chain *chain)
{
	...
	list_for_each_entry(entity, &chain->entities, chain) {
		...
		} else if (UVC_ENTITY_TYPE(entity) == UVC_EXT_PRIVACY_STUB_UNIT) {
			bmControls = entity->stub.bmControls;
			bControlSize = entity->stub.bControlSize;
		}
